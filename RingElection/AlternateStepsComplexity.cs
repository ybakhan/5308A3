...
{
    public static class AlternateStepsComplexity
    {
        public static void Main(string[] args)
        {
            Console.Write("*************************Alternate steps complexity test*************************" +
                              "\nThis test will output message cost and time cost of alternate steps for random rings" +
            ...
            Console.WriteLine("Alternate steps complexity test started");
            ...
            for (var i = 3; i <= maxRingSize; i++)
            {
                ...
                var ring = new Ring(ids.Select(id => new AlternateSteps(id)));
            ...
            var file = string.Format("AlternateSteps_{0}{1}{2}{3}{4}.csv",
                timeNow.Minute, timeNow.Hour, timeNow.Day, timeNow.Month, timeNow.Year);
            exporter.ExportToFile(file);

            Console.WriteLine("Alternate steps complexity test finished. Results saved in {0}", file);
        }
    }
}
