...
{
    public static class AsFarAsWorstCase
    {
        public static void Main(string[] args)
        {
            Console.Write("*************************As far as worst case test*************************" +
                  "\nThis test will output message cost and time cost of as far as in worst case" +
            ...
            Console.WriteLine("As far as worst case test started");

            var results = new List<TestResult>();
            for (var i = 3; i <= maxRingSize; i++)
            {
                var result = new TestResult { NodeCount = i };
                var ids = GetWorstCaseIDs(i);
                var min = ids.Min();

                var ring = new Ring(ids.Select(id => new AsFarAs(id)));
            ...
            var file = string.Format("AsFarAsWorstCase_{0}{1}{2}{3}{4}.csv",
                timeNow.Minute, timeNow.Hour, timeNow.Day, timeNow.Month, timeNow.Year);
            exporter.ExportToFile(file);

            Console.WriteLine("As far as worst case test finished. Results saved in {0}", file);
        }

        private static List<int> GetWorstCaseIDs(int count)
        {
