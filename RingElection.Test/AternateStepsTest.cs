...
{
    [TestClass]
    public class AternateStepsTest
    {
        ...
        {
            var nodes = new List<AlternateSteps>();
            for (int i = 1; i <= n; i++)
                nodes.Add(new AlternateSteps(i));

            var expectedLeader = nodes.ElementAt(0);
            var network = new Ring(nodes);

            AlternateSteps actualLeader;
            if (allInitiate)
            {
                actualLeader = network.Elect() as AlternateSteps;
            }
            else
            {
                var initiators = nodes.Shuffle().Take(rnd.Next(1, n)).Select(node => node.Id);
                actualLeader = network.Elect(initiators) as AlternateSteps;
            }
